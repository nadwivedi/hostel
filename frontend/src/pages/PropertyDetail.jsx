...
function PropertyDetail() {
  ...
  const getTenantPaymentInfo = (tenantId) => {
    const tenantPayments = payments.filter(p => p.tenantId?._id === tenantId || p.tenantId === tenantId);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 px-1 py-1.5 sm:p-4">
      <div className="w-full sm:max-w-7xl mx-auto space-y-1.5 sm:space-y-4">
        ...
        <div className="bg-white rounded-lg sm:rounded-2xl shadow-sm border border-gray-200 p-2 sm:p-5">
          <h1 className="text-sm sm:text-2xl font-black text-gray-800">
            {location.propertyName || location.name} <span className="text-gray-400 font-bold">-</span> <span className="font-bold text-gray-500">{location.location}</span>
          </h1>
        </div>
          ...
          <div className="bg-white rounded-md sm:rounded-2xl shadow-sm border border-purple-500 p-1.5 sm:p-5 transform hover:scale-105 transition-transform">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-sm sm:text-3xl font-black text-purple-600">{occupiedBeds}/{totalBeds}</h3>
              </div>
          ...
          <div className="bg-white rounded-md sm:rounded-2xl shadow-sm border border-red-500 p-1.5 sm:p-5 transform hover:scale-105 transition-transform">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-sm sm:text-3xl font-black text-red-600">₹{pendingPayments.toLocaleString()}</h3>
              </div>
          ...
          <div className="flex flex-row gap-1.5 sm:gap-2 items-center">
            <div className="relative flex-1">
              <input
                ...
                className="w-full pl-7 sm:pl-10 pr-2 py-2 sm:py-2.5 text-xs sm:text-sm border-2 border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-400 transition-all bg-white font-medium"
              />
      ...
      <div className="space-y-2 sm:space-y-5">
        {filteredRooms.map((room) => {
          ...
          return (
            <div key={room._id} className="bg-white rounded-lg sm:rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600 p-2 sm:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <div>
                      <h2 className="text-base sm:text-3xl font-black text-white drop-shadow-lg">
                        Room {room.roomNumber}
                      </h2>
                      <div className="flex items-center gap-1 sm:gap-2 mt-0.5 sm:mt-2 flex-wrap">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-1.5 sm:mb-3">
                          <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div>
                              <div className="text-xs sm:text-lg font-black text-gray-900">{tenant.name}</div>
                          ...
                          <div className="flex items-center gap-0.5 sm:gap-2" onClick={(e) => e.stopPropagation()}>
                            {tenant.bedNumber && (
                              <div className="px-1.5 py-0.5 sm:px-3 sm:py-1.5 bg-blue-100 rounded-md sm:rounded-xl">
                                <div className="text-[7px] sm:text-xs text-blue-600 font-bold">BED</div>
                                <div className="text-sm sm:text-xl font-black text-blue-700">{tenant.bedNumber}</div>
                              </div>
                            )}
                            <button onClick={() => handleEdit(tenant)} className="p-1 sm:p-2 text-blue-600 hover:bg-blue-50 rounded-md transition" title="Edit">
                              <svg className="w-3.5 h-3.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                              </svg>
                            </button>
                            {tenant.status === 'ACTIVE' && (
                              <button onClick={() => handleMarkAsLeft(tenant)} className="p-1 sm:p-2 text-orange-600 hover:bg-orange-50 rounded-md transition" title="Mark as Left">
                                <svg className="w-3.5 h-3.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="grid grid-cols-2 gap-1 sm:gap-3 mb-1.5 sm:mb-3">
                          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-md sm:rounded-xl p-1.5 sm:p-3 border border-green-200">
                            <div className="text-[8px] sm:text-xs text-green-600 font-bold uppercase">Rent</div>
                            <div className="text-sm sm:text-xl font-black text-green-700">₹{tenant.rentAmount?.toLocaleString()}</div>
                          </div>
                          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-md sm:rounded-xl p-1.5 sm:p-3 border border-blue-200">
                            <div className="text-[8px] sm:text-xs text-blue-600 font-bold uppercase">Joined</div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1 sm:gap-2">
                                {paymentInfo.type === 'pending' ? (
                                  <>
                                    ...
                                    <span className="text-[8px] sm:text-xs font-bold text-red-700 uppercase">
                                      Pending
                                    </span>
                                  </>
                                ) : (
                                  <>
                                    <svg className="w-2.5 h-2.5 sm:w-4 sm:h-4 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                                    </svg>
                                    <span className="text-[8px] sm:text-xs font-bold text-gray-600 uppercase">
                                      Paid
                                    </span>
                                  </>
                                )}
                              ...
                              <div className="flex items-center gap-1.5 sm:gap-3">
                                <div className="text-right">
                                  ...
                                  <div className={`text-xs sm:text-lg font-black ${
              ...
              {roomPending > 0 && room.rentType !== 'PER_BED' && (
                <div className="bg-gradient-to-r from-red-100 to-orange-100 border-t border-red-300 p-2 sm:p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 sm:gap-2">
                      <div className="w-1.5 h-1.5 sm:w-3 sm:h-3 rounded-full bg-red-600 animate-pulse"></div>
                      <span className="text-[10px] sm:text-sm font-bold text-red-800 uppercase">
                        Room Pending
                      </span>
                    </div>
                    <div className="text-base sm:text-2xl font-black text-red-700">
